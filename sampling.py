...
ANNULUS_AREA_LOC = '/constants/annulus_area'
BASE_MINUS_ONE_LOC = '/constants/base_minus_one'

N_SYSTEMS_LOC = '/settings/sim/n_systems'   
N_SAMPLES_LOC = '/settings/sim/n_samples'
NUMBER_OF_STEPS_LOC = '/settings/sim/number_of_steps'

WINDING_DIFF_RES_BIG_X_LOC = '/data/windings_diff_squared_x/big'
WINDING_DIFF_RES_SMALL_X_LOC = '/data/windings_diff_squared_x/small'

WINDING_SUM_RES_BIG_X_LOC = '/data/windings_sum_squared_x/big'
WINDING_SUM_RES_SMALL_X_LOC = '/data/windings_sum_squared_x/small'

WINDING_DIFF_RES_BIG_Y_LOC = '/data/windings_diff_squared_y/big'
WINDING_DIFF_RES_SMALL_Y_LOC = '/data/windings_diff_squared_y/small'

WINDING_SUM_RES_BIG_Y_LOC = '/data/windings_sum_squared_y/big'
WINDING_SUM_RES_SMALL_Y_LOC = '/data/windings_sum_squared_y/small'

ANNULUS_SUM_LOC = '/data/annulus_sum'
PARTITION_FUNCTION_LOC = '/data/partition_function'

SIZE_X_ARR_LOC = '/results/size_x'
SIZE_Y_ARR_LOC = '/results/size_y'
N_SAMPLES_ARR_LOC = '/results/n_samples'
...
WINDINGS_THERM_LOC = '/results/windings'
...
n_samples_arr = np.zeros(n_systems)
...
windings_1_therm_x = np.zeros((n_systems, number_of_steps))
windings_1_therm_y = np.zeros((n_systems, number_of_steps))
windings_2_therm_x = np.zeros((n_systems, number_of_steps))
windings_2_therm_y = np.zeros((n_systems, number_of_steps))
...
for l in range(n_systems):
    ...
    for i in range(n_samples):
        ...
        with h5.File(file_path, "r") as sim_file:
            base = np.array(sim_file[BASE_MINUS_ONE_LOC], dtype=np.uint64).astype(dtype=object) + 1
            annulus_area = np.array(sim_file[ANNULUS_AREA_LOC])
            ann_sum[i] = np.array(sim_file[ANNULUS_SUM_LOC], dtype=np.uint64) / annulus_area
            part_f[i] = np.array(sim_file[PARTITION_FUNCTION_LOC], dtype=np.int64)
            windings_diff_s_x[i] = (np.array(sim_file[WINDING_DIFF_RES_BIG_X_LOC], dtype=np.uint64) * base + 
                                        np.array(sim_file[WINDING_DIFF_RES_SMALL_X_LOC], dtype=np.uint64))
            windings_sum_s_x[i] = (np.array(sim_file[WINDING_SUM_RES_BIG_X_LOC], dtype=np.uint64) * base + 
                                        np.array(sim_file[WINDING_SUM_RES_SMALL_X_LOC], dtype=np.uint64))
            windings_diff_s_y[i] = (np.array(sim_file[WINDING_DIFF_RES_BIG_Y_LOC], dtype=np.uint64) * base + 
                                        np.array(sim_file[WINDING_DIFF_RES_SMALL_Y_LOC], dtype=np.uint64))
            windings_sum_s_y[i] = (np.array(sim_file[WINDING_SUM_RES_BIG_Y_LOC], dtype=np.uint64) * base + 
                                        np.array(sim_file[WINDING_SUM_RES_SMALL_Y_LOC], dtype=np.uint64))

    size_x_arr[l] = size_x
    size_y_arr[l] = size_y
    n_samples_arr[l] = n_samples
...
with h5.File(dist_name, "w") as dist_file:
    ...
    dist_file[N_SAMPLES_ARR_LOC] = n_samples_arr
